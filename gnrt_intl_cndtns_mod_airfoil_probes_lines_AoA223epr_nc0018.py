...
if pressure_taps=="yes":
    ...
    for i, point in enumerate(press_taps_coords_upper['offset_points']):
    ...
    for i, point in enumerate(press_taps_coords_lower['offset_points']):
        print(f"Pressure tap {i+1} coordinates: ({point[0]:.2f}, {point[1]:.2f}, {point[2]:.2f})")
        #probes.append( (point[0], point[1], point[2]) )
    ...
    plt.scatter(press_taps_coords_upper['offset_points'][:, 0], press_taps_coords_upper['offset_points'][:, 1], color='green', marker="+" ,label='Offset Points')
    plt.scatter(press_taps_coords_lower['offset_points'][:, 0], press_taps_coords_lower['offset_points'][:, 1], color='green', marker="+" ,label='Offset Points')
...
if probes_in_holes=="yes":
    ...
    axis_ul = ( origin_airfoil[0],   origin_airfoil[1] , origin_airfoil[2])
    
    for i, points in enumerate(press_taps_coords_upper['offset_points']):
        ...
        rp = rotate_press_taps(point, axis, angle)
       
        x_rot, y_rot ,z_rot = rp
 
        
        probes.append(rp)
        
    
    for i, points in enumerate(press_taps_coords_lower['offset_points']):
        
       
        point = (points[0], points[1], points[2])
        axis = axis_ul  # Rotate around the z-axis
        angle = np.radians(AoA)  
...
epsi["epsi"].sel(z=loc_init_z+((width/chord)/2), method="nearest").plot(x="x",cbar_kwargs={'orientation': 'horizontal'})
plt.gca().set_aspect('equal', adjustable='box')
plt.plot(axis[0],axis[1],color="red",marker="+",markersize=3)
...
epsi["epsi"].sel(y=loc_centr_y, method="nearest").plot(x="x",cbar_kwargs={'orientation': 'horizontal'})
plt.gca().set_aspect('equal', adjustable='box')
